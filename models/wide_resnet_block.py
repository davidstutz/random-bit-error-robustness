import torch
import common.torch
from .utils import get_normalization2d
...
class WideResNetBlock(torch.nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dropout=0, normalization='bn', init_scale=1, include_bias=False):
        """
        Constructor.
        ...
        common.torch.kaiming_normal_(self.conv1.weight, mode='fan_out', nonlinearity=activation, scale=self.init_scale)
        ...
        common.torch.kaiming_normal_(self.conv2.weight, mode='fan_out', nonlinearity=activation, scale=self.init_scale)
        if self.include_bias:
