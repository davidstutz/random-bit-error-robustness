...
def int_hamming_distance(a, b):
    ...
    assert (b.dtype == numpy.int32)

    assert len(a.shape) == len(a.shape)
    for d in range(len(a.shape)):
        assert a.shape[d] == b.shape[d]

    dist = numpy.zeros_like(a)
    n = dist.size

    _n = cffi.ffi.cast('int', n)
    _a = cffi.ffi.cast('int*', a.ctypes.data)
    _b = cffi.ffi.cast('int*', b.ctypes.data)
    _dist = cffi.ffi.cast('int*', dist.ctypes.data)
